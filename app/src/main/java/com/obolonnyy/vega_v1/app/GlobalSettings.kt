...
class GlobalSettings {
    ...
    companion object {
        ...
        fun setBeginningStudyDate(ctx: Context, date: String) {
            if (date.length > 15){
                throw Exception("Слишком большая длина")
            }
            val myDate = MyDateClass.dateParse(date)
            sPref = ctx.getSharedPreferences("myFileForData", 0)
            val editor = sPref.edit()
            editor.putString(beginningStudyDate, myDate.toString())
            editor.apply()
        }
    }
}